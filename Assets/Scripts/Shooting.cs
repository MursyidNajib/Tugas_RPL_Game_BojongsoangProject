...
public class Shooting : MonoBehaviour {
    public Transform generatedBulletFolder, bullet, eagle;
    public bool isTemplate, isNPC;
    public int player;
    public AudioSource shotSound, gameOver;
    ...
    void Update(){
        if ( !isTemplate && canShoot() && !anim.GetBool("hit") && 
            (((!isNPC && player == 1 && Input.GetKeyDown(KeyCode.Space)))
            || isNPC)){
            alreadyShot++;
            if (isNPC) StartCoroutine(DelayShootingFor(0.2f));
            else{SendMessage("scoreMin1"); LaunchBullet();}
        }
    }
